...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>Contents<small>Edit Contents</small></h1>
        </section>

        <!-- Main content -->
        <section class="content">
            <div class="row">
                {!! Form::open(['action'=>['ContentsController@update',$contents->id],'method'=>'PUT','files'=>true]) !!}
                <div class="col-xs-9">
                    <div class="box">
                        <!-- form start -->
                        <div class="box-body">
                            <div class="form-group">
                                ...
                                {!! Form::select('institution_id', $institutions, $contents->institution_id,['class'=>'form-control','required'=>'required']) !!}
                            </div>
                            <div class="form-group">
                                <label>Class</label>
                                @php $classes->prepend('Select Class','') @endphp
                                {!! Form::select('class_id', $classes, $contents->class_id,['class'=>'form-control','required'=>'required']) !!}
                            </div>
                            <div class="form-group">
                                <label>Subject</label>
                                @php $subject->prepend('Select Subject','') @endphp
                                {!! Form::select('subject_id', $subject, $contents->subject_id,['class'=>'form-control','required'=>'required']) !!}
                            </div>
                            <div class="form-group">
                                <label>Content Section</label>
                                @php $content_section->prepend('Select Content Section','') @endphp
                                {!! Form::select('content_section_id', $content_section, $contents->content_section_id,['class'=>'form-control','required'=>'required']) !!}
                            </div>
                            <div class="form-group">
                                <label>Content Type</label>
                                @php $content_type->prepend('Select Content Type','') @endphp
                                {!! Form::select('content_type_id', $content_type, $contents->content_type_id,['class'=>'form-control','required'=>'required']) !!}
                            </div>
                            <div class="form-group">
                                <label for="name">Content Name</label>
                                <input type="text" name="content_name" value="{{ $contents->content_name }}" placeholder="Enter Content Name" id="name" class="form-control" required>
                            </div>

                            <div class="form-group">
                                <label for="exampleInputEmail1">Detail</label>
                                <textarea name="file_content" cols="40" rows="12" id="DetailNews" data-error-container="#editor1_error" class="form-control" >{{$contents->file_content}}</textarea>
                            </div>
                        </div>
                        <!-- /.box-body -->
